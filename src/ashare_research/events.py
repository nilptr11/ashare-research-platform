...
def _load_pandas() -> Any:
    try:
        ...
        raise AStockEventDependencyError("A 股事件能力需要 pandas") from error
    return pd
...
def _load_akshare() -> tuple[Any, Any]:
    try:
        ...
        raise AStockEventDependencyError("A 股公告/业绩预告需要 akshare 和 pandas") from error
    return ak, pd
...
def parse_yyyymmdd(value: str | date | datetime | None = None) -> date:
    ...
    try:
        ...
        raise AStockEventError(f"日期必须是 YYYYMMDD 或 YYYY-MM-DD：{value}") from error
...
def auto_periods(anchor: str | date | datetime | None = None, count: int = 5) -> list[str]:
    ...
    return [period.strftime("%Y%m%d") for period in sorted(set(periods), reverse=True)[:count]]
...
def _notice_missing_code_error(error: Exception) -> bool:
    return isinstance(error, KeyError) and str(error).strip("\"'") == "代码"


def _fetch_notice_by_day(ak: Any, pd: Any, days: int, anchor: date, category: str, timeout: int, verbose_source: bool) -> Any:
    frames = []
    for offset in range(days):
        day = anchor - timedelta(days=offset)
        try:
            df_day = _run_with_retries(
                lambda day=day: ak.stock_notice_report(symbol=category, date=day.strftime("%Y%m%d")),
                timeout=timeout,
                verbose_source=verbose_source,
            )
        except Exception as error:
            if _notice_missing_code_error(error):
                continue
            raise
...
def fetch_notice(
    ...
    if stock:
        try:
            ...
            if not _notice_missing_code_error(error):
                raise
            df = _fetch_notice_by_day(ak, pd, days, anchor, category, timeout, verbose_source)
            df = df[df["代码"].astype(str) == str(stock)] if "代码" in df.columns else pd.DataFrame()
    else:
        df = _fetch_notice_by_day(ak, pd, days, anchor, category, timeout, verbose_source)
...
def fetch_forecast(
    ...
    for period in resolved_periods:
        try:
            df_period = _run_with_retries(lambda period=period: ak.stock_yjyg_em(date=period), timeout=timeout, verbose_source=verbose_source)
        except Exception as error:
            failures.append(f"{period}: {type(error).__name__}: {error}")
            continue
...
def _run_with_retries(call: Callable[[], Any], *, timeout: int, verbose_source: bool, retries: int = 2) -> Any:
    last_error: Exception | None = None
    for attempt in range(retries + 1):
        try:
            return run_akshare(call, timeout=timeout, verbose_source=verbose_source)
        except Exception as error:
            last_error = error
            if attempt >= retries:
                break
...
def prepare_forecast(df: Any, keyword: str | None = None, stock: str | None = None, start: date | None = None, end: date | None = None) -> Any:
    pd = _load_pandas()
    if df is None or df.empty:
        return pd.DataFrame()
    result = df.copy()
