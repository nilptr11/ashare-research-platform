...
feature、evidence、knowledge 和 run 留痕等分析数据输入的读取关系见 [分析数据目录](../architecture/analysis-data-catalog.md)。

日常维护默认使用同一个可重入入口，不分收盘版和晚间版。`daily run` 会刷新默认基础库、构建 `5/20/60` feature，并生成质量报告；基础库本身长期保留，不做 30/60/120 日滚动删除。更长窗口属于分析读取选择，可在 feature 构建或 mart 读取时显式指定。
...
| 全市场短线/题材分析 | 先确认 `daily status`，再按需要读取 mart/feature |
| 趋势、量价结构、风格回看 | 按用户框架读取对应窗口的 mart/feature |
| 个股或候选池财务验证 | 先按明确股票池和报告期补相应财务 mart，再读公告、财务、evidence 和 knowledge |
