...
基础数据目录见 [基础数据维护对照表](../operations/maintenance-dataset-catalog.md)。Codex 默认先读根目录 `SKILL.md` 和 `codex/data-map.md`；本表只说明各能力适合做什么，不规定固定研究流程。
...
| Capability Registry | `ashare capabilities list/show/validate` | 内置 capability specs | capability spec | 帮 Codex 按问题选择数据和判断边界 | 不执行工作流，不生成结论，不替代数据地图 |
| Context Pack | `ashare context compose`、`ashare context build market-structure/industry/stock` | mart、feature、evidence、knowledge、capability specs | `data/context_packs/.../context.json` | 生成可选快照和下钻便利包 | 不作为默认研究入口，不直接生成结论，不替代 Codex 判断 |
| Protocol 模板 | `ashare protocols list/show/validate/output-schema` | registered protocol specs | protocol spec 和输出 schema | 沉淀反复使用的输出结构、质量门和 suggested capabilities；`industry_chain_selection.v1` 用于主线选股、产业链拆解和候选池分层 | 不替代用户当次框架，不输出交易执行指令 |
| Run 留痕 | `ashare runs record/list/replay` | question、capability、context、evidence、knowledge、输出 | `runs/<run_id>` | 保存分析上下文、能力卡片、artifact hash 和质量门，便于复盘 | 不回流为事实源 |

## 推荐读取顺序

| 分析场景 | 推荐顺序 |
| --- | --- |
| 全市场短线、题材、风格强弱 | `daily status` 确认可分析 -> 读相关 mart/feature -> 已有快照时再用 context 辅助 |
| 产业方向判断 | 读行业/概念 feature 和 mart -> 搜索或入库外部 evidence -> 用龙头和成交趋势验证 |
| 主线选股与产业链拆解 | 读 `codex/data-map.md` -> 下钻 mart/feature -> 补产业链 evidence/knowledge -> 用 `industry_chain_selection.v1` 组织输出 |
...
LLM 默认不应该直接读 raw store，也不应该现场拼散接口。默认入口是用户问题、`SKILL.md` 和 `codex/data-map.md`；数据读取优先级是 mart/feature、evidence、knowledge，context pack 只作为可选快照。
...
| `context_pack` | `pack_id`、`as_of`、`coverage`、`facts`、`inputs`、`data_gaps`、`source_policy_summary` | 可选事实快照和下钻便利包 |
